...
public final class MurmurHash {
    ...
    public static long[] hash64(MemorySegment key, int offset, int length, long seed) {
        ...
        switch (length & 15) {
            ...
            case 9:
                k2 ^= ((long) key.get(ValueLayout.JAVA_BYTE, advancedOffset + 8)) << 0;
                k2 *= c2;
                k2 = rotl64(k2, 33);
                k2 *= c1;
                h2 ^= k2;
    ...
    private static long getBlock(MemorySegment key, int offset, int index) {
        ...
        return ((long) key.get(ValueLayout.JAVA_BYTE, blockOffset + 0) & 0xff)
                + (((long) key.get(ValueLayout.JAVA_BYTE, blockOffset + 1) & 0xff) << 8)
                + (((long) key.get(ValueLayout.JAVA_BYTE, blockOffset + 2) & 0xff) << 16)
                + (((long) key.get(ValueLayout.JAVA_BYTE, blockOffset + 3) & 0xff) << 24)
                + (((long) key.get(ValueLayout.JAVA_BYTE, blockOffset + 4) & 0xff) << 32)
                + (((long) key.get(ValueLayout.JAVA_BYTE, blockOffset + 5) & 0xff) << 40)
                + (((long) key.get(ValueLayout.JAVA_BYTE, blockOffset + 6) & 0xff) << 48)
                + (((long) key.get(ValueLayout.JAVA_BYTE, blockOffset + 7) & 0xff) << 56);
    }
